...
//                System.out.println("%%% capacity = " + capacity);
		...
		cloudlet.setCloudletLength(length);
                long cap = (new Double(capacity)).longValue();
//                System.out.println("%%%%cap = " + cap);
                long extraTime = (long)extraSize / cap;
//                System.out.println("%%%ExtraTime = " + extraTime);
                
                cloudlet.setExtraTime(extraTime); //Tempo extra em segundos
                cloudlet.setExtraSize((long)extraSize); //Tamanho extra em MI
	...
	public double getTotalUtilizationOfCpu(double time) {
        ...
        public double getTotalUtilizationOfCpuPercentage(double time) {
		double totalUtilization = 0;
		for (ResCloudlet gl : getCloudletExecList()) {
			totalUtilization += gl.getCloudlet().getUtilizationOfCpu(time);
		}
                
                if (totalUtilization>1) totalUtilization = 1;
		return totalUtilization;
	}
	...
	public double getCurrentRequestedUtilizationOfBw() {
        ...
        public double getCurrentRemainingMips(int vmId) {

        //Essa é a linha MÁGICA que não deixa as tarefas adiantarem!
        //Huhuhuhohohohahahaha!!! =D
        this.updateVmProcessing(CloudSim.clock(), getCurrentMipsShare());
        
        double soma = 0.0; //somatório dos MIPs restantes...
        int contCloudlet = 0;
        for (ResCloudlet rcl : getCloudletExecList()) {
            if (rcl.getCloudlet().getVmId() == vmId) {
                soma += rcl.getRemainingCloudletLength();
                contCloudlet++;
            }
        }
    ...
    //    System.out.println("--- antes - soma waiting = " + soma);
        for (ResCloudlet rcl : getCloudletWaitingList()) {
            if (rcl.getCloudlet().getVmId() == vmId) {
                soma += rcl.getRemainingCloudletLength();
                contCloudlet++;
      ...
      //  System.out.println("--- depois - soma waiting = " + soma);
      //  System.out.println("--- VM # " + vmId + " Quant. Cloudlets contadas=" + contCloudlet + " | MIPs referentes = " + soma);
