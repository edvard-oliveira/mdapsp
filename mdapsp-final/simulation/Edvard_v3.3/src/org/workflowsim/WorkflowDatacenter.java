...
public class WorkflowDatacenter extends Datacenter {
    ...
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
        ...
        Log.printLine("Datacenter # " + this.getId() + " - Processando a execução da Cloudlet no WorkFlowDatacenter!!");
        Log.printLine("---");
        Log.printLine("Número de VMs do Datacenter " + this.getId() + " = " + this.getVmList().size());
        ...
        try {
            ...
            if (job.isFinished()) {
                ...
                Log.printLine("Enviando retorno do JOB");
...
//        System.out.println("Datacenter.checkCloudletCompletion...");
        List<? extends Host> list = getVmAllocationPolicy().getHostList();
        for (Host host : list) {
//            System.out.println("for da lista de Hosts");
            for (Vm vm : host.getVmList()) {
//                System.out.println("For da lista de VMs");
                while (vm.getCloudletScheduler().isFinishedCloudlets()) {
//                    System.out.println("WHILE - FinishedCloudlets");
                    Cloudlet cl = vm.getCloudletScheduler().getNextFinishedCloudlet();
                    if (cl != null) {
//                        System.out.println("Disparou Datacenter.CloudletRETURN");
                        sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);
                        register(cl);
                    } else {
//                        System.out.println("Não há próxima Cloudlet!!");
                    }
                }
            }
        }
    ...
    //**********************************************************************
    //Métodos adicionados: 07/11/2017

    public double getFileTransferTime(List<FileItem> requiredFiles, Job job) {
        try {
            return processDataStageInForComputeJob(requiredFiles, job);
        } catch (Exception ex) {
            Logger.getLogger(WorkflowDatacenter.class.getName()).log(Level.SEVERE, null, ex);
        }
        return -1;
    }
}
